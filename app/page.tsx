...
import { poster, backdrop, TMDBMovie, getGenreNames } from '@/lib/tmdb'
...
const CONTINUE_WATCHING = [
  { title: 'Arcane', sub: 'S2 E3 · 40%', p: 40, c: '#10081f', tc: '#c07cf7' },
  { title: 'Breaking Bad', sub: 'S5 E8 · 72%', p: 72, c: '#0c1808', tc: '#7cf78a' },
  { title: 'One Piece', sub: 'Ep 847 · 55%', p: 55, c: '#0a1020', tc: '#7cb4f7' },
]
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [trendingRes, popularRes, topRatedRes, animeRes, nowPlayingRes] = await Promise.all([
          ...
          fetch(`/api/tmdb?endpoint=movie/now_playing`),
        ])
        ...
        const np = nowPlayingData.results?.map(transformMovie) || []
  ...
  useEffect(() => {
    ...
    if (readyCount > 0 && !testedRef.current.has(currentProvider) || providerStates[currentProvider]?.status !== 'ready') {
      selectFastest()
    }
  }, [providerStates])
  ...
  const getFilteredMovies = () => {
    let movies = [...popular]
    if (selectedFilter === 'Movies') movies = movies.filter(m => m.type === 'movie')
    else if (selectedFilter === 'TV Shows') movies = movies.filter(m => m.type === 'tv')
    return movies
  }
    ...
    <>
      ...
      <div className={`notif-dropdown ${notifOpen ? 'open' : ''}`}>
        <div className="nd-head">
          <h3>Notifications</h3>
          <span onClick={() => showToast('All marked as read ✓')}>Mark all read</span>
        </div>
        <div className="notif-item">
          <div className="notif-dot2"></div>
          <div className="notif-item-text">
            <h4>New Episode Available</h4>
            <p>Breaking Bad — Episode 62 is now streaming</p>
        ...
        <div className="notif-item">
          ...
          <div className="notif-item-text">
            <h4>Coming Soon</h4>
            <p>New releases arriving this week</p>
      ...
      <div className={`page ${currentPage === 'home' ? 'active' : ''}`} id="page-home">
        <div className="hero">
          ...
          <div className="hero-content">
            <div className="hero-eyebrow">Featured — Now Streaming</div>
            ...
            <div className="hero-chips">
              ...
              {heroItem?.genre.slice(0, 2).map((g, i) => (
                <span key={i} className="hchip">{g}</span>
              ))}
              <span className="hdot"></span>
              <span className="hchip">⭐ {heroItem?.rating?.toFixed(1) || '0.0'}</span>
            </div>
            <p className="hero-desc">{heroItem?.description || 'Discover unlimited entertainment on VaultSphere.'}</p>
        ...
        <div className="home-sections">
          ...
          <div className="row-wrap">
            <div className="row-head">
              <span className="row-label">Continue <em>Watching</em></span>
              <span className="row-all">See all</span>
            </div>
            <div className="cards-scroll">
              {CONTINUE_WATCHING.map((item, i) => (
                <div key={i} className="wcard" onClick={() => showToast('Resuming playback...')}>
                  <div className="wcard-img">
                    <div style={{ width: '100%', height: '100%', background: item.c, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: item.tc, opacity: 0.55 }}>{item.title}</span>
                    </div>
                    <div className="wcard-play">
                      <div className="wcard-play-inner">
                        <svg viewBox="0 0 24 24"><path d="M5 3l14 9-14 9V3z"/></svg>
                      </div>
                    </div>
                  </div>
                  <div className="pbar"><div className="pfill" style={{ width: `${item.p}%` }}></div></div>
                  <div className="wcard-body">
                    <div className="wcard-title">{item.title}</div>
                    <div className="wcard-sub">{item.sub}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="row-wrap">
            <div className="row-head">
              <span className="row-label">Trending <em>Now</em></span>
              <span className="row-all">See all</span>
            </div>
            <div className="cards-scroll">
          ...
          <div className="row-wrap">
            <div className="row-head">
              <span className="row-label">Popular <em>Movies</em></span>
              <span className="row-all" onClick={() => goToPage('browse')}>See all</span>
            </div>
            <div className="cards-scroll">
          ...
          <div className="row-wrap">
            <div className="row-head">
              <span className="row-label">Top <em>Rated</em></span>
              <span className="row-all">See all</span>
            </div>
            <div className="cards-scroll">
          ...
          <div className="row-wrap">
            <div className="row-head">
              <span className="row-label">Animation</span>
              <span className="row-all">See all</span>
            </div>
            <div className="cards-scroll">
        ...
        <footer className="site-footer">
          <div className="ft-grid">
            <div className="ft-brand">
              <span className="ft-logo">VAULT<span>SPHERE</span></span>
              <p className="ft-tagline">Your gateway to unlimited entertainment.</p>
            </div>
          ...
          <div className={`player-video-area ${playing ? 'playing' : ''}`} style={{ cursor: 'pointer' }}>
            <div className="player-fake-video">
              {selectedMovie?.backdrop && (
                <img src={selectedMovie.backdrop} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.3 }} />
              )}
            </div>
            ...
            <div className="player-controls-overlay">
              <div className="player-progress">
                <div className="player-buffered" style={{ width: '78%' }}></div>
                <div className="player-progress-fill" style={{ width: `${prog}%` }}></div>
              </div>
              <div className="player-ctrl-row">
                ...
                <button className="ctrl-btn" title="Fullscreen" onClick={() => showToast('Fullscreen mode')}>
          ...
          <div className="player-info-panel">
            <div className="player-title-area">
              ...
              <div className="player-chips">
                <span className="hchip">⭐ {selectedMovie?.rating.toFixed(1)}</span>
              </div>
          ...
          <div className="episodes-panel">
            ...
            <div className="cards-scroll" style={{ gap: 8 }}>
              {providers.map((p, i) => (
                ...
                >
                  <span>{p.name}</span>
                  {providerStates[i]?.status === 'ready' && (
                    <span style={{ fontSize: 10, color: '#4ade80' }}>✓</span>
                  )}
                  {providerStates[i]?.status === 'testing' && (
      ...
      <div className={`page ${currentPage === 'search' ? 'active' : ''}`} id="page-search">
        <div className="search-page">
          <div className="search-top">
            <h1>Find <span>Anything</span></h1>
            <p>Millions of movies, shows, anime, documentaries and more</p>
          </div>
          <div className="big-search">
            <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <input
              type="text"
              placeholder="Search titles, genres, actors, directors…"
              value={searchQuery}
          ...
          {searchQuery.length >= 2 ? (
            <div className="search-results-grid" style={{ maxWidth: 980, margin: '0 auto' }}>
            ...
            <>
              ...
              <div className="genre-mosaic">
                {GENRES.map((g) => (
                  <div
                    key={g.name}
                    className="gmosaic-card"
                    data-icon={g.icon}
                    style={{ background: g.bg, borderColor: `${g.c}22`, color: g.c }}
            ...
            <div className="detail-hero">
              ...
              <div className="detail-content">
                <div className="badge-row">
                  <span className="imdb-b">IMDb</span>
                  <span className="star-row">★★★★☆</span>
                  <span style={{ color: 'var(--accent)', fontWeight: 700, fontSize: 14 }}>{selectedMovie.rating.toFixed(1)}</span>
                  <span className="rt-badge">HD</span>
                </div>
                <h1 className="detail-title">{selectedMovie.title}</h1>
                <div className="detail-chips">
                  <span className="dchip">{selectedMovie.year}</span>
                  {selectedMovie.genre.slice(0, 3).map((g, i) => (
                    <span key={i} className="dchip">{g}</span>
                  ))}
                </div>
            ...
            <div className="detail-body">
              <div>
                <div className="detail-tabs">
                  <span className="dtab active">Details</span>
                  <span className="dtab">Cast</span>
                  <span className="dtab">Reviews</span>
                </div>
                ...
                {trending.slice(1, 6).map((item) => (
                  <div key={item.id} className="sim-item" onClick={() => { setSelectedMovie(item); setCurrentPage('detail') }}>
                    ...
                    <div className="sim-info">
                      <h4>{item.title}</h4>
                      <p>{item.year}</p>
                      <div className="sim-rate">⭐ {item.rating.toFixed(1)}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className={`page ${currentPage === 'community' ? 'active' : ''}`} id="page-community">
        <div className="community-wrap">
          <div className="comm-header">
            ...
            <p>Discover what the world is watching, rating, and reviewing right now.</p>
          </div>
          <div className="comm-layout">
            <div>
              <div className="feed-tabs">
                <span className="ftab active">For You</span>
                <span className="ftab">Popular</span>
                <span className="ftab">New Reviews</span>
              </div>
              <div className="activity-card">
                ...
                <div className="act-movie-strip" onClick={() => trending[1] && handleWatch(trending[1])}>
                  ...
                  <div className="act-movie-info">
                    <h4>{trending[1]?.title || 'Loading...'}</h4>
                    <p>{trending[1]?.year}</p>
                  </div>
                </div>
                ...
                {trending.slice(0, 5).map((item, i) => (
                  <div key={item.id} className="trend-list-item" onClick={() => handleWatch(item)}>
                    ...
                    <div className="trend-details">
                      <h4>{item.title}</h4>
                      <p>{item.genre[0]} · {item.year}</p>
                    </div>
                    <div className="trend-thumb-sm">
                      {item.poster ? (
                        <img src={item.poster} alt="" />
                      ) : (
                        <div style={{ width: '100%', height: '100%', background: '#131920' }}></div>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className={`page ${currentPage === 'myspace' ? 'active' : ''}`} id="page-myspace">
        <div className="myspace-wrap">
          <div className="profile-banner">
            <div className="profile-banner-decoration">VS</div>
          </div>
          <div className="profile-main">
            <div className="profile-card">
              ...
              <div className="profile-name-area">
                <h1>Alex Viewer</h1>
                <p>Member since 2024 · VaultSphere Member</p>
              </div>
              <div className="profile-stats-row">
                <div className="pstat"><div className="pstat-num">247</div><div className="pstat-label">Watched</div></div>
                <div className="pstat"><div className="pstat-num">84</div><div className="pstat-label">Watchlist</div></div>
          ...
          <div className={`ptab-content ${profileTab === 'watchlist' ? 'active' : ''}`} id="pt-watchlist">
          ...
          <div className={`ptab-content ${profileTab === 'history' ? 'active' : ''}`} id="pt-history">
            <div className="history-list">
              {CONTINUE_WATCHING.map((item, i) => (
                <div key={i} className="hi" onClick={() => showToast('Resuming...')}>
                  <div className="hi-thumb">
                    <div style={{ width: '100%', height: '100%', background: item.c, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 14, color: item.tc, opacity: 0.7 }}>{item.title}</span>
                    </div>
                  </div>
                  <div className="hi-info">
                    <h4>{item.title}</h4>
                    <p>{item.sub}</p>
                    <div className="hi-prog pbar" style={{ marginTop: 6 }}><div className="pfill" style={{ width: `${item.p}%` }}></div></div>
                  </div>
                  <div className="hi-when">Recently</div>
                </div>
              ))}
            </div>
          </div>

          <div className={`ptab-content ${profileTab === 'reviews' ? 'active' : ''}`} id="pt-reviews">
          ...
          <div className={`ptab-content ${profileTab === 'prefs' ? 'active' : ''}`} id="pt-prefs">
            <div className="pref-section">
              <h3>Playback</h3>
              <div className="pref-row">
                <div className="pref-label">
                  <h4>Auto-play next episode</h4>
                  <p>Automatically start the next episode when one ends</p>
                </div>
              ...
              <div className="pref-row">
                <div className="pref-label">
                  ...
                  <p>Preferred streaming quality</p>
                </div>
                <select className="select-pref">
                  <option>Auto</option>
            ...
            <div className="pref-section">
              ...
              <div className="pref-row">
                <div className="pref-label">
                  <h4>New episodes</h4>
                  <p>Get notified when new episodes drop</p>
                </div>
                <label className="toggle">
                  <input type="checkbox" defaultChecked />
                  <span className="toggle-slider"></span>
        ...
        <div className="settings-wrap">
          ...
          <div className="settings-body">
              ...
              <div className="settings-group">
                <div className="settings-row">
                  <div className="sr-label">
                    <h4>Display Name</h4>
                    <p>Shown on reviews and community posts</p>
                  </div>
                  <div className="sr-control">
                    <input className="form-input" defaultValue="Alex Viewer" style={{ width: 200, padding: '8px 12px', fontSize: 13 }} />
                  </div>
                </div>
                <div className="settings-row">
                  <div className="sr-label">
                    <h4>Email Address</h4>
                    <p>Used for sign-in and notifications</p>
                  </div>
                  <div className="sr-control">
                    <input className="form-input" defaultValue="alex.viewer@email.com" style={{ width: 240, padding: '8px 12px', fontSize: 13 }} />
                  </div>
                </div>
                <div className="settings-row">
                  <div className="sr-label">
                  ...
                  <div className="sr-control">
                    <button className="btn btn-outline btn-sm" onClick={() => showToast('Password reset email sent ✓', 'accent')}>Change Password</button>
                  </div>
                </div>
              </div>
            ...
            <div className={`s-panel ${settingsPanel === 'subscription' ? 'active' : ''}`}>
              <h2>Subscription</h2>
              <p>Your plan, billing, and payment methods</p>
              <div className="settings-divider"></div>
            ...
            <div className={`s-panel ${settingsPanel === 'playback' ? 'active' : ''}`}>
              <h2>Playback</h2>
              <p>Control how videos play across your devices</p>
              <div className="settings-divider"></div>
              <div className="settings-group">
                <div className="settings-row">
                  <div className="sr-label">
                ...
                <div className="settings-row">
                  <div className="sr-label">
                    <h4>Auto-play Next Episode</h4>
                    <p>Start next episode automatically</p>
                  </div>
                  <label className="toggle">
                    <input type="checkbox" defaultChecked />
                    <span className="toggle-slider"></span>
            ...
            <div className={`s-panel ${settingsPanel === 'privacy' ? 'active' : ''}`}>
              ...
              <p>Control your data and account security settings</p>
              <div className="settings-divider"></div>
              <div className="settings-group">
                <div className="settings-row">
                  <div className="sr-label">
                    <h4>Watch History</h4>
                    <p>Allow tracking for recommendations</p>
                  </div>
                  <label className="toggle">
                    <input type="checkbox" defaultChecked />
                    <span className="toggle-slider"></span>
                ...
                <div className="settings-row">
                  <div className="sr-label">
                    ...
                    <p>Let others see your activity</p>
                  </div>
                  <label className="toggle">
                    <input type="checkbox" defaultChecked />
                    <span className="toggle-slider"></span>
